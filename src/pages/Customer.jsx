...
export default function Customer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await apiRequest(`/customer/${editingId}`, {
          method: 'PUT',
          body: JSON.stringify(payload)
        });
        markItemAsUpdated('customer', editingId);
        Swal.fire('Success', 'Customer updated!', 'success');
      } else {
        await apiRequest('/customer/create-customer', {
          method: 'POST',
          body: JSON.stringify(payload)
        });
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You want to delete this customer directory listing?",
      icon: 'warning',
    ...
    <div className="space-y-4">
      ...
      <div className="hidden print:block mb-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 border border-slate-300 rounded-lg p-2 bg-slate-50 text-left">
          ...
          <div className="px-2 py-0.5">
            <span className="block text-[8px] font-extrabold text-slate-500 uppercase tracking-wider">Total Registered Customers</span>
            <span className="text-[11px] font-black text-slate-900">{filteredCustomers.length} Customers</span>
          </div>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-1.5 sm:gap-2 print:hidden">
        <div className="flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-brand-accent uppercase mb-1">
            ...
            <span className="text-slate-400 truncate">Customers</span>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredCustomers.length === 0 ? (
                ...
                filteredCustomers.map((c) => (
                  <tr key={c.customerId} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-5 py-3 font-bold text-slate-800 text-xs">{c.customerName}</td>
                    <td className="px-5 py-3 font-mono text-[11px] text-slate-600">{c.phone || 'N/A'}</td>
                    <td className="px-5 py-3 text-slate-600 text-xs">{c.address || 'N/A'}</td>
      ...
      <section className="space-y-3.5 lg:hidden">
        {filteredCustomers.length === 0 ? (
          ...
          filteredCustomers.map((c) => (
            <article key={c.customerId} className="p-[1px] rounded-2xl bg-gradient-to-br from-blue-500/30 via-slate-200 to-indigo-500/30 shadow-md shadow-slate-200/60 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 overflow-hidden">
              <div className="bg-[#fff7f9] rounded-2xl overflow-hidden flex flex-col h-full">
                ...
                <div className="relative bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 px-4 py-3.5 text-white flex items-center justify-between overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white backdrop-blur-md border border-white/10 shadow-inner">
                      <span className="font-black text-base text-blue-300 font-mono">{(c.customerName || 'C')[0]}</span>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-blue-200 font-mono tracking-wider">CUST-{c.customerId}</span>
                      </div>
                    </div>
                  </div>
                  <span className="relative z-10 inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider backdrop-blur-md border bg-blue-500/20 text-blue-300 border-blue-500/30">
                    Client
                  </span>
                </div>
                ...
                <div className="p-4 bg-white text-xs space-y-2 text-slate-600">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono font-bold text-slate-800 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">{c.phone || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-slate-400">Address:</span> 
                    <span className="font-semibold text-slate-700 truncate max-w-[200px]">{c.address || 'N/A'}</span>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-brand-accent focus:ring-2 focus:ring-brand-accent/20 rounded-xl px-4 py-2.5 text-sm text-slate-900 font-bold transition-all"
                  placeholder="e.g. pandiyan@example.com"
                />
              </div>

              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Delivery Address</label>
                <textarea
                  value={form.address}
