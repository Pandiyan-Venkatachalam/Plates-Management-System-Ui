...
import { Plus, Trash2, Search, ChevronRight, Pencil, Ruler } from 'lucide-react';
...
export default function Unit() {
  ...
  const loadData = () => {
    apiRequest('/unit')
      .then(res => setUnits(sortLatestFirst(res.data, ['unitId', 'id'], 'unit')))
      .catch(console.error);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await apiRequest(`/unit/${editingId}`, {
          method: 'PUT',
          body: JSON.stringify({ unitName: name })
        });
        markItemAsUpdated('unit', editingId);
        ...
        Swal.fire('Success', 'Unit created!', 'success');
      }
      setName('');
      setEditingId(null);
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You want to delete this unit listing?",
      icon: 'warning',
    ...
    try {
      await apiRequest(`/unit/${id}`, { method: 'DELETE' });
      Swal.fire('Deleted!', 'Unit deleted!', 'success');
      loadData();
    } catch (err) {
      Swal.fire('Error', err.message, 'error');
    }
  ...
  const filteredUnits = sortLatestFirst(
    units.filter(u => 
    ...
    <div className="space-y-4">
      ...
      <div className="hidden print:block mb-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 border border-slate-300 rounded-lg p-2 bg-slate-50 text-left">
          ...
          <div className="px-2 py-0.5">
            <span className="block text-[8px] font-extrabold text-slate-500 uppercase tracking-wider">Total Units Registered</span>
            <span className="text-[11px] font-black text-slate-900">{filteredUnits.length} Units</span>
          </div>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-1.5 sm:gap-2 print:hidden">
        <div className="flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-brand-accent uppercase mb-1">
            ...
            <span className="text-slate-400 truncate">Units</span>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 text-white">
              <tr className="border-b border-slate-800">
                ...
                <th className="px-5 py-3 text-[10px] font-bold text-slate-200 uppercase tracking-widest whitespace-nowrap w-1/2">Unit Name</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredUnits.length === 0 ? (
                ...
                filteredUnits.map((u) => (
                  <tr key={u.unitId} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-5 py-3 font-mono font-bold text-[10px] text-slate-500">UNT-{u.unitId}</td>
                    <td className="px-5 py-3 font-bold text-slate-800 text-xs">{u.unitName}</td>
      ...
      <section className="space-y-3.5 lg:hidden">
        {filteredUnits.length === 0 ? (
          ...
          filteredUnits.map((u) => (
            <article key={u.unitId} className="p-[1px] rounded-2xl bg-gradient-to-br from-blue-500/30 via-slate-200 to-indigo-500/30 shadow-md shadow-slate-200/60 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-blue-200 font-mono tracking-wider">UNT-{u.unitId}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-backdrop-in">
          <div className="animate-modal-pop bg-[#ffeef1] border border-pink-200/80 w-full max-w-md max-h-[90vh] rounded-2xl p-5 shadow-2xl flex flex-col overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Unit Name</label>
                <input
                  type="text"
                  value={name}
