...
import { Plus, Trash2, Search, ChevronRight, Pencil, Sliders } from 'lucide-react';
...
export default function Variant() {
  ...
  const loadData = () => {
    apiRequest('/variant')
      .then(res => setVariants(sortLatestFirst(res.data, ['variantId', 'id'], 'variant')))
      .catch(console.error);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await apiRequest(`/variant/${editingId}`, {
          method: 'PUT',
          body: JSON.stringify({ variantName: name })
        });
        markItemAsUpdated('variant', editingId);
        Swal.fire('Success', 'Variant updated!', 'success');
      } else {
        await apiRequest('/variant', {
          method: 'POST',
          body: JSON.stringify({ variantName: name })
        });
        Swal.fire('Success', 'Variant created!', 'success');
      }
      setName('');
      setEditingId(null);
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You want to delete this variant listing?",
      icon: 'warning',
    ...
    try {
      await apiRequest(`/variant/${id}`, { method: 'DELETE' });
      Swal.fire('Deleted!', 'Variant deleted!', 'success');
      loadData();
    } catch (err) {
      Swal.fire('Error', err.message, 'error');
    }
  ...
  const filteredVariants = sortLatestFirst(
    variants.filter(v => 
    ...
    <div className="space-y-4">
      ...
      <div className="hidden print:block mb-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 border border-slate-300 rounded-lg p-2 bg-slate-50 text-left">
          ...
          <div className="px-2 py-0.5">
            <span className="block text-[8px] font-extrabold text-slate-500 uppercase tracking-wider">Total Product Variants</span>
            <span className="text-[11px] font-black text-slate-900">{filteredVariants.length} Variants</span>
          </div>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-1.5 sm:gap-2 print:hidden">
        <div className="flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-brand-accent uppercase mb-1">
            ...
            <span className="text-slate-400 truncate">Variants</span>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 text-white">
              <tr className="border-b border-slate-800">
                ...
                <th className="px-5 py-3 text-[10px] font-bold text-slate-200 uppercase tracking-widest whitespace-nowrap w-1/2">Variant Name</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredVariants.length === 0 ? (
                ...
                filteredVariants.map((v) => (
                  <tr key={v.variantId} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-5 py-3 font-mono font-bold text-[10px] text-slate-500">VAR-{v.variantId}</td>
                    <td className="px-5 py-3 font-bold text-slate-800 text-xs">{v.variantName}</td>
      ...
      <section className="space-y-3.5 lg:hidden">
        {filteredVariants.length === 0 ? (
          ...
          filteredVariants.map((v) => (
            <article key={v.variantId} className="p-[1px] rounded-2xl bg-gradient-to-br from-blue-500/30 via-slate-200 to-indigo-500/30 shadow-md shadow-slate-200/60 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-blue-200 font-mono tracking-wider">VAR-{v.variantId}</span>
                      </div>
                    </div>
                  </div>
