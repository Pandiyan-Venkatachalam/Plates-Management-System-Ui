...
export default function Category() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await apiRequest(`/category/${editingId}`, {
          method: 'PUT',
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You want to delete this category listing?",
      icon: 'warning',
    ...
    <div className="space-y-4">
      ...
      <div className="hidden print:block mb-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 border border-slate-300 rounded-lg p-2 bg-slate-50 text-left">
          ...
          <div className="px-2 py-0.5">
            ...
            <span className="text-[11px] font-black text-slate-900">{filteredCategories.length} Categories</span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 text-white">
              <tr className="border-b border-slate-800">
                ...
                <th className="px-5 py-3 text-[10px] font-bold text-slate-200 uppercase tracking-widest whitespace-nowrap w-1/2">Category Name</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredCategories.length === 0 ? (
                ...
                filteredCategories.map((c) => (
                  <tr key={c.categoryId} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-5 py-3 font-mono font-bold text-[10px] text-slate-500">CAT-{c.categoryId}</td>
                    <td className="px-5 py-3 font-bold text-slate-800 text-xs">{c.categoryName}</td>
      ...
      <section className="space-y-3.5 lg:hidden">
        {filteredCategories.length === 0 ? (
          ...
          filteredCategories.map((c) => (
            <article key={c.categoryId} className="p-[1px] rounded-2xl bg-gradient-to-br from-blue-500/30 via-slate-200 to-indigo-500/30 shadow-md shadow-slate-200/60 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-blue-200 font-mono tracking-wider">CAT-{c.categoryId}</span>
                      </div>
                    </div>
                  </div>
